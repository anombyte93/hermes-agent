...
2. 显示**证据**——该会话最后一条已存储的助手回复，直接从转录中读取（末尾的工具输出绝不会被当作证据）。如果该回复早于目标本身，命令会以 `stale_evidence` 拒绝——它属于该会话更早的目标，对它裁判可能造成虚假的 done。
3. 用循环使用的同一个裁判、同样的子目标/完成契约上下文，对该证据进行裁判。真实运行时质量门禁先行，与循环中的顺序一致；`--dry-run` 下门禁**完全推迟**——它们是 shell 命令，只读报告不得执行它们。
...
| 结果 | 含义 | 退出码 |
|---|---|---|
| `done` | 裁判判定完成且所有门禁通过；目标已转换（无门禁目标使用 `--dry-run` 时本应转换） | 0 |
| `gates_pending` | 带门禁目标的 `--dry-run`：裁判判定完成，但门禁**未被评估**（未运行）——只是预览，不是认定 | 0 |
| `already_done` | 目标已完成——未裁判、未更改 | 0 |
| `incomplete` | 已裁判：目标确实未完成（或某个质量门禁失败；门禁耗尽重试还会可见地暂停目标） | 1 |
| `stale_evidence` | 最新存储的回复早于目标——拒绝裁判更早目标的遗留内容 | 1 |
| `goal_changed` | 裁判调用期间目标被编辑；未写入任何内容——请重新运行 | 1 |
| `no_session` / `no_goal` | 会话不存在，或该会话没有目标——未裁判 | 2 |
| `unreachable` | 裁判 API 仍然无法访问 | 3 |
| `unparseable` | 裁判有回复，但不是可用的判定 | 4 |

安全属性：不运行任何 agent 轮次、绝不重置轮次预算、绝不重放工具，所有状态更改都走目标循环使用的同一条持久化路径。`--dry-run` 不运行、不写入任何内容——门禁命令不会被执行，门禁重试预算不会被消耗，重复 dry-run 会让存储的目标逐字节保持不变。写入完成转换之前会重读目标：如果裁判调用期间目标被他人重设、清除或完成，转换会被拒绝而不是覆盖它。**其他**会话的目标绝不会被触碰——命名了错误的会话只会得到 `no_goal`，而不是意外的状态转换。完成转换后重复执行是幂等的（`already_done`，不调用裁判）。
